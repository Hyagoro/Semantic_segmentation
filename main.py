...
def load_vgg(sess, vgg_path):
    ...
    return input_tensor, keep, l3, l4, l7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function, make a fully connected neural network DONE
    ...
    vgg_layer4_out_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    l11 = tf.add(l1, vgg_layer4_out_1x1)

    l2 = tf.layers.conv2d_transpose(l11, num_classes, 4, strides=(2, 2), padding='same',
                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    vgg_layer3_out1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1,
                                         padding='same',
                                         kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    l21 = tf.add(l2, vgg_layer3_out1x1)

    l3 = tf.layers.conv2d_transpose(l21, num_classes, 16, strides=(8, 8), padding='same',
                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    correct_label = tf.reshape(correct_label, (-1, num_classes))  # try
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, train_op, cross_entropy_loss = optimize(layer_output, correct_label=correct_label,
                                                        learning_rate=learning_rate, num_classes=num_classes)

        # TODO: Train NN using the train_nn function
        train_nn(sess, nb_epoch, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
                 correct_label, keep_prob, learning_rate)
